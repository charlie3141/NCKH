// Vietnamese character mapping tables
const TableManager = {
    tableA: [
    ...
    vietnameseTable: [   {key: "IAY", forms: ["iáy", "iày", "iảy", "iãy", "iạy", "iay", "iấy", "iầy", "iẩy", "iẫy", "iậy", "iây", "iắy", "iằy", "iẳy", "iẵy", "iặy", "iăy"]},
...
class VietnameseConverter {
    constructor() {
        ...
        this.vietnameseCache = new Map();
        this.translationCache = {};
    }

    getMappingForIndices(mpuState, a0, a1, a2, a3) {
        ...
        const result = this.getMappingForIndicesOriginal(mpuState, a0, a1, a2, a3);
        this.mappingCache.set(cacheKey, result);
        return result;
    }

    getMappingForIndicesOriginal(mpuState, a0, a1, a2, a3) {
        if (mpuState < 0 || mpuState > 7) return null;
        
        if (a3 === 2) {
            if (a0 < 3 && a1 < 3 && a2 < 3) {
                return TableManager.tableC[a0][a1][a2];
            }
            return null;
        }
        ...
        if (flatIndex >= 0 && flatIndex < 9) {
            return a3bin === 0 ? TableManager.tableA[mpuState][flatIndex] : TableManager.tableB[mpuState][flatIndex];
        }
        
        return null;
    }

    convertVietnameseWord(encodedWord) {
        if (!encodedWord || encodedWord.length === 0) return "";
        
        if (this.vietnameseCache.has(encodedWord)) {
            return this.vietnameseCache.get(encodedWord);
        }
        
        const result = this.convertVietnameseWordOriginal(encodedWord);
        this.vietnameseCache.set(encodedWord, result);
        return result;
    }

    convertVietnameseWordOriginal(encodedWord) {
        if (!encodedWord || encodedWord.length === 0) return "";
        ...
        for (let i = 0; i < TableManager.vietnameseTable.length; i++) {
            if (vowelKeyUpper === TableManager.vietnameseTable[i].key) {
                const convertedVowel = TableManager.vietnameseTable[i].forms[tableIndex];
    ...
    convertVietnameseText(encodedText) {
        if (!encodedText || encodedText.length === 0) return "";
        
        const words = encodedText.split(' ');
        const convertedWords = words.map(word => this.convertVietnameseWord(word));
        return convertedWords.join(' ');
    }

    getMPUState(orientation, shakeState) {
    ...
    clearCache() {
        ...
        this.vietnameseCache.clear();
        this.translationCache = {};
    }
}

// Create global instance
const vietnameseConverter = new VietnameseConverter();
